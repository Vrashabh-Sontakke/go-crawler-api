...
func Router() {
	...
	router.HandleFunc("/{vin}", engineCrawler)
...
func engineCrawler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Println("Error getting Engine Page Link")
	}

	// Crawler for actual Engine Page
	engines, err := getEngines(*link)
	if err != nil {
		log.Println("Error getting engines.")
	}
...
func getEngines(url_suffix string) ([]Output, error) {
	var engines []Output
	c := colly.NewCollector(
		colly.AllowURLRevisit(),
	)
...
func getIndividualEngineData(links []Output) ([]Output, error) {
